...
export class InventoryService {
  ...
  async createDefault(userId: string, productId: string, dto: CreateVariationDefaultDto): Promise<VariationEntity> {
    const [foundUser, foundProduct, foundVariation] = await Promise.all([
      this.userRepository.findOne({ id: userId }),
      this.productRepository.findOne({ where: { id: productId }, relations: ['user'] }),
      this.repository.findOne({ where: { name: dto.name, product: { id: productId } } })
    ])
  ...
  async createSize(userId: string, productId: string, dto: CreateVariationSizeDto): Promise<VariationEntity> {
    const [foundUser, foundProduct, foundVariation] = await Promise.all([
      this.userRepository.findOne({ id: userId }),
      this.productRepository.findOne({ where: { id: productId }, relations: ['user'] }),
      this.repository.findOne({ where: { size: dto.size, product: { id: productId } } })
    ])
  ...
  async createColor(userId: string, productId: string, dto: CreateVariationColorDto): Promise<VariationEntity> {
    const [foundUser, foundProduct, foundVariation] = await Promise.all([
      this.userRepository.findOne({ id: userId }),
      this.productRepository.findOne({ where: { id: productId }, relations: ['user'] }),
      this.repository.findOne({ where: { color: dto.color, product: { id: productId } } })
    ])
  ...
  async updateDefault(userId: string, id: string, dto: UpdateVariationDefaultDto): Promise<VariationEntity> {
    const [foundUser, foundVariation] = await Promise.all([
      this.userRepository.findOne({ id: userId }),
      this.repository.findOne({ where: { id }, relations: ['product', 'product.user'] })
    ])
  ...
  async updateSize(userId: string, id: string, dto: UpdateVariationSizeDto): Promise<VariationEntity> {
    const [foundUser, foundVariation] = await Promise.all([
      this.userRepository.findOne({ id: userId }),
      this.repository.findOne({ where: { id }, relations: ['product', 'product.user'] })
    ])
  ...
  async updateColor(userId: string, id: string, dto: UpdateVariationColorDto): Promise<VariationEntity> {
    const [foundUser, foundVariation] = await Promise.all([
      this.userRepository.findOne({ id: userId }),
      this.repository.findOne({ where: { id }, relations: ['product', 'product.user'] })
    ])

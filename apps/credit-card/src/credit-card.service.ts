...
import { CreditCardUtilsService } from '@app/utils/adapters/credit-card/credit-card-utils.service';
...
export class CreditCardService {
  ...
  async create(userId: string, dto: CreateCreditCardDto): Promise<CreditCardEntity> {
    ...
    const alreadyExists = foundCCs.find(CC => dto.number === this.cypervService.decrypt(CC.number))
    ...
    return {
      ...newCC,
      cvc: '***',
      number: this.cypervService.decrypt(newCC.number).replace(/.(?=.{4})/g, "*")
    } 
  }
  ...
  async findById(id: string): Promise<CreditCardEntity> {
    ...
    return {
      ...foundCC,
      cvc: '***',
      number: this.cypervService.decrypt(foundCC.number).replace(/.(?=.{4})/g, "*")
    } 
  }

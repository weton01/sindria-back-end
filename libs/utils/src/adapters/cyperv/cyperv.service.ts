...
export class CypervService {
  ...
  constructor(
    ...
    if(!options.ALGORITHM)
      throw new Error('ALGORITHM of cyperv is required')
    if(!options.ENCODING)
      throw new Error('ENCODING of cyperv is required')
    if(!options.IV_LENGTH)
      throw new Error('IV_LENGTH of cyperv is required')
    if(!options.KEY)
      throw new Error('KEY of cyperv is required')
  ...
  public encrypt(data: string) {
    ...
    const cipher = crypto.createCipheriv(this.ALGORITHM,  Buffer.from(sha256.digest()), iv);
  ...
  public decrypt(data: string) {
    ...
    const decipher = crypto.createDecipheriv(this.ALGORITHM,  Buffer.from(sha256.digest()), iv);
